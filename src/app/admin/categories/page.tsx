...
export default function AdminCategories() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold text-[#111111] mb-2">Manajemen Kategori</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 h-fit">
          <h2 className="font-bold text-[#111111] mb-4 flex items-center gap-2">
            <Plus size={18} className="text-[#025246]" /> {editing ? "Edit Kategori" : "Tambah Kategori"}
          </h2>
          <form action={formAction} className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={isPending}
                className="flex-1 rounded-xl bg-[#025246] py-3 text-sm font-bold text-white hover:bg-[#024036] disabled:opacity-50"
              >
        ...
        <div className="lg:col-span-2">
          <div className="space-y-3">
            {categoryList.map((c) => (
              <div key={c.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-[#025246]/5 flex items-center justify-center text-2xl flex-shrink-0">
                  {c.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-gray-900">{c.name}</h3>
                  <p className="text-xs text-gray-500 truncate">{c.description || "-"}</p>
                  <p className="text-xs text-[#025246] mt-1 font-medium">{c.count} komoditas</p>
                </div>
