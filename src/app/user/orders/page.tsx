...
export default function CartPage() {
  ...
  // side effect (network call + log) WAJIB di dalam useEffect, deps kosong -> sekali doang
  useEffect(() => {
    const run = async () => {
      const user = getClientUser();
      const userId = Number(user?.id);
      if (!userId) return; // user belum login, jangan lanjut

      const items = getCart();
      if (items.length === 0) return; // cart kosong, nggak ada yang perlu diproses

      // 1. bikin order session dulu
      const orderResult = await getUsersOrder(userId);
      if (!orderResult.success || !orderResult.data) {
        console.error("gagal bikin order:", orderResult.message);
        return;
      }
      const orderUserId = orderResult.data.id;

      // 2. insert SEMUA item cart, bukan cuma yang terakhir
      ...
      results.forEach((r, i) => {
        if (!r.success) {
          console.error("gagal insert item:", items[i], r.message);
        }
      });

      console.log("selesai migrasi cart ke DB", results);
    };
