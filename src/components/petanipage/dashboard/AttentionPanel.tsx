  ...
  return (
    ...
    >
      ...
      <p className="mb-4 text-xs text-gray-400">
        Prioritas yang perlu Anda tangani sekarang.
      </p>
      {items.length === 0 ? (
        <div className="flex items-center gap-3 rounded-2xl border border-success/20 bg-success/5 px-4 py-4">
        ...
        <ul className="divide-y divide-gray-100">
          {items.map((item) => (
            <li
              key={item.key}
              className="flex flex-col gap-2 py-3.5 first:pt-0 sm:flex-row sm:items-center sm:justify-between sm:gap-4"
            >
              <div className="flex min-w-0 items-start gap-3">
                <span
                  className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${TONE_ICON[item.tone]}`}
                >
                  {item.icon}
                </span>
